...
var StatusNameMapReceivedKey = map[string]string{
	"50":  "รอรับกุญแจ",
	"50e": "เกินวันนัดหมาย",
}
...
func (h *ReceivedKeyHandler) SearchRequests(c *gin.Context) {
	...
	var statusNameMap = StatusNameMapReceivedKey
...
func (h *ReceivedKeyHandler) GetRequest(c *gin.Context) {
	funcs.GetAuthenUser(c, h.Role)
	funcs.GetRequest(c, StatusNameMapReceivedKey)
}
...
// @Router /api/received-key/update-key-pickup-driver [put]
func (h *ReceivedKeyHandler) UpdateKeyPickupDriver(c *gin.Context) {
	...
	var trnRequest models.VmsTrnReceivedKeyDriver
	...
	var result models.VmsTrnReceivedKeyDriver
...
// @Router /api/received-key/update-key-pickup-pea [put]
func (h *ReceivedKeyHandler) UpdateKeyPickupPEA(c *gin.Context) {
	user := funcs.GetAuthenUser(c, h.Role)
	...
	var trnRequest models.VmsTrnReceivedKeyPEA
	if err := config.DB.First(&trnRequest, "trn_request_uid = ?", request.TrnRequestUID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Booking not found"})
		return
	}
	...
	var result models.VmsTrnReceivedKeyPEA
	if err := config.DB.First(&result, "trn_request_uid = ?", request.TrnRequestUID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Booking not found"})
		return
	}
...
// @Router /api/received-key/update-key-pickup-outsider [put]
func (h *ReceivedKeyHandler) UpdateKeyPickupOutSider(c *gin.Context) {
	user := funcs.GetAuthenUser(c, h.Role)
	...
	var trnRequest models.VmsTrnReceivedKeyOutSider
	if err := config.DB.First(&trnRequest, "trn_request_uid = ?", request.TrnRequestUID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Booking not found"})
		return
	}
	...
	var result models.VmsTrnReceivedKeyOutSider
	if err := config.DB.First(&result, "trn_request_uid = ?", request.TrnRequestUID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Booking not found"})
		return
	}
...
func (h *ReceivedKeyHandler) UpdateCanceled(c *gin.Context) {
	user := funcs.GetAuthenUser(c, h.Role)
	var request models.VmsTrnRequestCanceled
	...
	var trnRequest models.VmsTrnRequestCanceled
	if err := config.DB.First(&trnRequest, "trn_request_uid = ?", request.TrnRequestUID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Booking not found"})
		return
	}
	...
	request.CanceledRequestEmpID = user.EmpID
	...
	var result models.VmsTrnRequestCanceled
	if err := config.DB.First(&result, "trn_request_uid = ?", request.TrnRequestUID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Booking not found"})
		return
	}

...
var StatusNameMapVehicelInUse = map[string]string{
	"51": "รับยานพาหนะ",
	"60": "เดินทาง",
}
...
func (h *VehicleInUseHandler) SearchRequests(c *gin.Context) {
	...
	statusNameMap := StatusNameMapVehicelInUse
...
func (h *VehicleInUseHandler) GetRequest(c *gin.Context) {
	funcs.GetAuthenUser(c, h.Role)
	funcs.GetRequest(c, StatusNameMapVehicelInUse)
}
...
func (h *VehicleInUseHandler) GetVehicleTripDetails(c *gin.Context) {
	...
	if err := config.DB.Find(&trips, "trn_request_uid = ? AND is_deleted = ?", trnRequestUid, "0").Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Trip not found"})
		return
	}
...
func (h *VehicleInUseHandler) GetVehicleAddFuelDetails(c *gin.Context) {
	...
	if err := config.DB.Where("trn_request_uid = ? AND is_deleted = ?", trnRequestUid, "0").Find(&fuels).Error; err != nil {
...
// @Router /api/vehicle-in-use/update-satisfaction-survey/{trn_request_uid} [put]
...
// @Router /api/vehicle-in-use/returned-vehicle [put]
func (h *VehicleInUseHandler) ReturnedVehicle(c *gin.Context) {
	user := funcs.GetAuthenUser(c, h.Role)
